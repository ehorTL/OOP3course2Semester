let camera, renderer, labelRenderer, scene, wSocket, connected, core;
let vertex = [];
let vertexMaterials = [];
let lineS = [], lineE = [];
let materialLines = [];
...
function init() {
    ...
    labelRenderer.setSize(window.innerWidth, window.innerHeight - document.getElementById("info").offsetHeight);
    labelRenderer.domElement.style.position = 'absolute';
    labelRenderer.domElement.style.top = document.getElementById("info").offsetHeight+"px";
    divForImage.appendChild(labelRenderer.domElement);

    new THREE.OrbitControls(camera, labelRenderer.domElement);

    //new THREE.OrbitControls(camera, renderer.domElement);
    new THREE.TransformControls(camera, labelRenderer.domElement);

}

function createSphere(x, y, z, i) {
    ...
    nDiv.textContent = 'N' + i;
    nDiv.style.marginTop = '0';
    var earthLabel = new CSS2DObject(nDiv);
